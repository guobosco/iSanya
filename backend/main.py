...
def build_seed_user_payload(index: int) -> dict:
    now = int(time.time() * 1000)
    # 每人独立昵称（用户名/展示名），与客户端广场种子达人风格一致并补足 20 人
    names = [
        "阿舟", "林晚", "小满", "陈厨", "阿轻",
        "周周", "南乔", "老韩", "沐沐", "大川",
        "安然", "铁柱", "小鹿", "阿K", "糖糖",
    ...
    jobs = [
        "普雷塞", "房源策展人", "旅行顾问", "空间设计师", "接待经理",
        "摄影向导", "语言教师", "文化讲解员", "生活方式博主", "社群主理人",
    ]
    schools = [
        "Ecole du Louvre, Paris", "Sorbonne Universite, Paris", "Sciences Po, Paris",
        "Universite de Paris", "Ecole Boulle, Paris"
    ]
    cities = ["巴黎", "里昂", "尼斯", "马赛", "图卢兹"]
    countries = ["法国", "意大利", "西班牙", "德国", "荷兰"]
    lang_sets = [
        ["法语", "英语"],
        ["意大利语", "英语"],
        ["西班牙语", "英语"],
    ...
    if 1 <= index <= len(names):
        ...
        name = f"种子用户{index:02d}"
    score = round(4.70 + (index % 25) * 0.01, 2)
    review_count = 120 + index * 19
    service_years = 2 + (index % 9)
    phone = f"1380000{index:04d}"
    return {
        "id": f"seed-user-{index:02d}",
        "pei_pei_id": f"pp_seed_{index:02d}",
        "name": name,
        "email": f"seed{index:02d}@lulu.app",
        "phone_number": phone,
        "hashed_password": get_password_hash("123456"),
        "photo_url": "",
        "remark_name": "",
        "tags": ["房东", "达人", f"城市向导{(index % 3) + 1}"],
        "favorite_service_ids": [],
        "signature": f"你好，我是{name}，欢迎来我的城市。",
        "memo": "",
        "gender": "女",
        "region": cities[index % len(cities)],
        "job_title": jobs[index % len(jobs)],
        "education": schools[index % len(schools)],
        "birth_decade": ["75后", "80后", "85后", "90后", "95后", "00后", "05后", "10后"][index % 8],
        "middle_school_favorite_song": [
            "晴天", "七里香", "后来", "遇见", "小幸运", "起风了", "海阔天空", "稻香", "平凡之路", "夜曲",
        ][index % 10],
        "spoken_languages": lang_sets[index % len(lang_sets)],
        "living_city": cities[index % len(cities)],
        "living_country": countries[index % len(countries)],
        "identity_verified": True,
        "review_count": review_count,
        "average_rating": score,
        "service_years": service_years,
        "review_summaries": [
            review_templates[index % len(review_templates)],
            review_templates[(index + 2) % len(review_templates)],
        ],
        "height_cm": 155 + (index % 35),
        "weight_kg": float(46 + (index % 30)),
        "height_weight_private": (index % 9 == 0),
        ...
        "last_id_modification_year": date.today().year,
        "created_at": now - index * 3600_000,
        "updated_at": now - index * 120_000,
        "is_synced": True,
    }


def ensure_seed_users(db: Session, total: int = 20):
    for index in range(1, total + 1):
        ...
        for key, value in payload.items():
            if key in {"id", "hashed_password", "created_at"}:
                continue
            if key in seed_identity_keys:
                setattr(existing, key, value)
                continue
            if getattr(existing, key, None) in (None, "", [], 0, 0.0, False):
                setattr(existing, key, value)
    db.commit()
...
def ensure_seed_discovery_services(db: Session, count: int = 100):
    ...
    for i in range(max(count, len(seed_titles))):
        ...
        extra = i // len(seed_titles)
        titles.append(base if extra == 0 else f"{base}（第{extra + 1}组）")
    locs = [
        "海南省三亚市 吉阳区",
        "海南省三亚市 海棠湾",
        "海南省三亚市 亚龙湾",
    ...
    desc_templates = [
        "【种子数据】提前预约时间、集合地点与注意事项；支持线上沟通细节，见面后按约定流程进行。",
        "【种子数据】适合初次体验者，会准备简单物料清单；如需改期请至少提前一天说明。",
        "【种子数据】可定制强度与节奏，过程中可随时调整；结束时可给简单复盘与建议。",
        "【种子数据】含路线与时长建议，不含门票与餐饮费用；雨天有备选室内方案。",
        "【种子数据】支持 1 对 1 或 2～4 人小团；欢迎自带设备或使用我方基础装备。",
        "【种子数据】工作日夜间与周末均可约；具体时段请在私信里协商。",
        "【种子数据】偏轻松社交向，也欢迎认真学习型；会尊重边界与隐私。",
        "【种子数据】开发联调专用文案，上线前请在管理端替换为真实服务说明。",
    ]
    price_opts = [
        "¥68/次", "¥99/次", "¥128/次", "¥168/次", "¥199/次", "¥260起", "¥388套餐",
        "面议", "¥79/小时", "¥120/小时", "¥200/半天", "¥450/天",
    ]
    mode_opts = ["到店", "上门", "线上", "到店或上门", "线上+到店"]
    seed_users = (
        db.query(models.User.id, models.User.name)
        .filter(models.User.id.startswith("seed-user"))
        .order_by(models.User.id)
        .all()
    )
    name_by_uid = {row.id: (row.name or "") for row in seed_users}
    now = int(time.time() * 1000)
    for i in range(1, count + 1):
        ...
        extra = (i - 1) // len(titles) + 1
        if extra > 1:
            title = f"{title}（第{extra}组）"
        desc = desc_templates[(i - 1) % len(desc_templates)]
        desc = f"{desc}\n服务编号：{sid}；发布者：{name_by_uid.get(creator_id, creator_id)}。"
        ...
        db.add(
            models.Service(
                ...
                image_urls=imgs,
                location=loc,
                price_text=price_opts[(i - 1) % len(price_opts)],
                price_basis_text="种子数据 · 计价以沟通确认为准",
                category=category,
                service_mode=mode_opts[(i - 1) % len(mode_opts)],
                service_time=int([60, 90, 120, 150, 180, 240, 360][i % 7]),
                sync_to_square=True,
                participant_ids=participants,
                creator=name_by_uid.get(creator_id, ""),
                creator_id=creator_id,
                is_important=(i % 10 == 0),
                created_at=now - i * 45_000,
                updated_at=now - i * 22_000,
                is_deleted=False,
                is_synced=True,
            )
        )
...
async def on_startup():
    ...
    db = SessionLocal()
    try:
        ensure_seed_users(db, total=20)
        ensure_seed_discovery_services(db)
    finally:
        db.close()
